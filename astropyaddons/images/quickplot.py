...
def quickplot(img: FITSImage, phi_lo: float=-1, phi_hi: float=3, corners: tuple[coord, coord]=None) -> None:
    ...
    Parameters:
      ...
      - `corners`: tuple containing 2 `coords`, each written as `(x, y)`.
    ...
    size_x, size_y = img.grid.size_x, img.grid.size_y
    median, std = img.grid.median, img.grid.std

    ### EXTRACTING `CORNERS` DATA
    # Extracting default
    ((x1, y1), (x2, y2)) = corners if corners is not None \
        else ((0, 0), (size_x-1, size_y-1))
    ...
    if x_min < 0 or y_min < 0 or x_max > size_x-1 or y_max > size_y-1:
        raise IndexError(f"Corners given are out of range of data. {(size_x, size_y)}")
    ...
    plt.imshow(
        img.grid.grid[x_min:x_max+1, y_min:y_max+1], 
        origin='lower', 
        cmap='gray',
        vmin=lo,
        vmax=hi
